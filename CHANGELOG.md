...
- Add opt-in strict naming: `strictNaming: true` on `PrometheusMeterProvider`
  (params `strict_naming`) applies the core `RegistrationGuard` — suffix
  conventions, conflicting re-registration and series collisions throw at
  registration. One guard is shared across scoped meters (#32).
- Require `rasuvaeff/yii3-metrics` `^2.3`.
